use crate::{
    ...
    find_model_with_retry, AccountInfo, DownloadMetadataInfo,
};
...
pub async fn handle_token2022_mint_account<'a, 'b, 'c>(
    ...
    if let Some(metadata) = &m.extensions.metadata {
        ...
        task = Some(DownloadMetadataInfo {
...
async fn upsert_asset(
    ...
    let  asset_model = asset::ActiveModel {
        ...
        burnt: Set(false),
        mint_extensions: Set(Some(if is_nft {extensions.to_owned()} else {extensions})),
        ..Default::default()
    };
